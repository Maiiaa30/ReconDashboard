...
- **Quality** - **GitHub Actions CI** on every push: **lint** + typecheck + unit **and route-level (`fastify.inject`) integration** tests (backend); **lint + Vitest/RTL tests** (incl. `axe` accessibility checks) + build + a **first-load bundle budget** (frontend). Responses are **runtime-validated with zod at the API boundary** (a backend shape change fails loud instead of rendering `undefined`), and a custom ESLint rule **bans un-guarded outbound `fetch()`** in the recon code so every target-facing request goes through the SSRF-guarded client - the safety conventions are enforced, not just documented
