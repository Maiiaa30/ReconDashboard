...
- 🎯 **Gated active scanning & confirmation** - `nmap`, `nuclei`, `ffuf` (recursion · vhost · fingerprint-aware wordlists), `sqlmap` and friends, locked behind `active_authorized`, an engagement scope (allow/deny) and an authorization window. Against a host already known to sit behind a WAF it works **WAF-aware**: sqlmap can opt into vendor-tailored `--tamper` evasion (auto-escalating on a still-blocked retry), while nuclei/ffuf/dalfox auto-throttle (rate-limit + jitter) so a WAF-fronted target isn't hammered. It also confirms what it finds: **nuclei-driven CVE verification** promotes a passive "CVE present" signal to *confirmed-exploitable* with the PoC attached, an **IDOR / broken-authorization** helper replays one object request under three identities to flag access-control gaps, **parameter discovery** finds honored-but-undocumented params, and the OWASP engine confirms **SSTI**, open-redirect and CORS with false-positive-killing differentials - never fired at an unauthorized target.
- 🧠 **Intelligence & triage** - deterministic rules-based scoring, **attack-path correlation** rendered as a network graph, an optional AI advisor, **suggest-only AI triage** (the LLM proposes a disposition per finding; you apply with one click - it never changes anything itself), and **immutable engagement report snapshots** you can **export to PDF**. It also **imports** externally-produced output - Nuclei JSONL, Nmap XML and generic findings JSON dedup into findings; host lists, httpx JSONL and URL lists load the subdomain estate / URL corpus (out-of-scope dropped, everything tagged `imported`) - so the dashboard is not a data silo.
...
| **Subdomains** | Passive discovery (crt.sh · certspotter · subfinder), HTTP-probe enrichment, **WAF/CDN detection** (a Cloudflare/Akamai/… badge) with one-click **WAF brand identification** (an **"Identify WAF"** button, `wafw00f`-backed - 150+ signatures, persisted on the host), **sortable by status / host / IP / last-seen**, diff & flag new, Discord alerts, exports. A host stuck behind a Cloudflare **"Just a moment…" 403** is re-probed through **headless Chromium** that solves the JS challenge, so a site that works in your browser shows as reachable here too | 🟢 passive |
...
| **Scans** | `nmap` (quick top-1000 · **deep = all ports + `-sV` + NSE scripts + OS detection**, with service/version, port state and script output · **attack-surface sweep** - one nmap per live host of the domain, deduped by IP) · `nuclei` (template-tag presets) · `ffuf` · **parameter discovery** (Arjun-style - finds honored-but-undocumented query params via chunk + bisect). Cloudflare-fronted targets are auto-cleared: the challenge is solved once in a headless browser and the `cf_clearance` cookie + browser UA are passed to nuclei/ffuf so they aren't fuzzing the interstitial - **gated, loud** | 🔴 active |
| **Tools** | `katana` · `naabu` · `dalfox` · `sslscan` · `sqlmap` (opt-in **WAF evasion** - identifies the WAF, auto-picks a per-vendor `--tamper` chain, raises level/risk, and **auto-escalates** to a heavier chain on a still-blocked retry) · WordPress enum · **403/401 bypass** (categorised technique battery - encoding / traversal / routing-header / verb / method-override, with a **soft-403 body-diff** so a 200-that-still-denies isn't a false win) · HTTP-method audit · exposed-datastore probes. Web-facing tools (katana/dalfox/sqlmap/wp-enum/403-bypass) ride a solved Cloudflare clearance when the target is challenged - **gated** | 🔴 active |
...
| **Replay** | A server-side **Repeater** - compose/edit and re-send any request (gzip/br/zstd-decoded, inert **Body** view + **sandboxed rendered Preview**), with per-target **history** you can re-open; a real **Intruder** - four attack modes (**sniper / battering-ram / pitchfork / cluster-bomb**) over numbered `{{P1}}...{{Pn}}` positions (`{{PAYLOAD}}` still works), fed by **lists / number ranges / curated wordlists / a saved payload library**, with **grep-extract & grep-match** result columns, **median + MAD** anomaly flagging and a bounded **concurrency** pool; an **Authz** mode that replays one `{{ID}}` object request under **three identities** (yours / a second account / anonymous) to surface **IDOR & broken access control** (every hit is *needs-review*, never auto-confirmed); session-wide **match/replace rules**; and a **Sitemap** tab that assembles the target's endpoint tree from captured requests + fuzz hits + discovery, one click loading any endpoint into the Repeater. A Cloudflare challenge on a sent request is **auto-solved** and replayed with the clearance cookie (flagged on the response), and the Intruder pre-warms it once so every payload rides it | 🔴 active |
...
- **Live updates** - a single session-guarded **server-sent-events** channel (`GET /api/events`) pushes coarse change signals (jobs · findings · runs) so the UI refreshes the relevant view immediately. It is layered *on top of* polling, never replacing it - a dropped stream degrades to the poll, never to stale data shown as fresh
...
- **Quality** - **GitHub Actions CI** on every push: **lint** + typecheck + unit **and route-level (`fastify.inject`) integration** tests (backend); **lint + Vitest/RTL tests** (incl. `axe` accessibility checks) + build + a **first-load bundle budget** (frontend). Responses are **runtime-validated with zod at both boundaries** - the backend validates flagged endpoints' responses on the way out (a preSerialization hook) and the frontend re-validates them on receipt, so a backend shape change fails loud instead of rendering `undefined`, and a custom ESLint rule **bans un-guarded outbound `fetch()`** in the recon code so every target-facing request goes through the SSRF-guarded client - the safety conventions are enforced, not just documented
...
- 🧷 **CSRF defense** - the session cookie is `SameSite=strict`, and in the single-origin production deployment a state-changing request whose `Origin` isn't on the `TRUSTED_ORIGINS` allowlist is **refused** (the token-authed capture ingest is exempt by design).
...
Locally you run `docker compose up` (the dev stack: Vite on `5173`, the API on `3001`). In production this sits on a private VM (Oracle Always Free / Hetzner / OVH) **behind Tailscale** - never exposed to the public internet, no public TLS/ACME by design.

Production uses a separate **single-origin** topology - [`docker-compose.prod.yml`](./docker-compose.prod.yml):

```bash
cp .env.example .env        # set ADMIN_PASSWORD, SESSION_SECRET, TRUSTED_ORIGINS
docker compose -f docker-compose.prod.yml up -d --build
```

- The frontend is **built to static assets and served by the backend**, so the whole app (UI + API) lives on **one origin and one tailnet port** - which also lets the `TRUSTED_ORIGINS` CSRF check apply.
- The backend is **compiled to a plain-Node bundle** (`dist/server.mjs`, esbuild) and run under Node - no `tsx`/TypeScript at runtime; `NODE_ENV=production`, no read-write source mounts, and a container **healthcheck** on `/api/health`.
- Keep an **encrypted backup** (Settings → Encrypted backup) off-box so a host suspension is never data loss.

See **[`docs/production.md`](./docs/production.md)** for the full runbook (bring-up, upgrade, backup/restore drill, rollback).
