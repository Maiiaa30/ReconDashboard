...
export async function certSpotterSubdomains(domain: string, signal?: AbortSignal): Promise<string[]> {
  const url =
    `https://api.certspotter.com/v1/issuances?domain=${encodeURIComponent(domain)}` +
    `&include_subdomains=true&expand=dns_names`

  const data = await getJson<CertSpotterIssuance[]>(url, { timeoutMs: 25_000, signal })
